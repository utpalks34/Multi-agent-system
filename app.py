...
if st.session_state.running and not st.session_state.done:
    ...
    # pipeline state key -> UI result key
    STATE_TO_UI = {
        "search_results": "search",
        "scraped_content": "reader",
        "report": "writer",
        "feedback": "critic",
    }
    STEP_MESSAGES = {
        "start": "🔍 Search Agent is working…",
        "search": "📄 Reader Agent is scraping top resources…",
        "reader": "✍️ Writer is drafting the report…",
        "writer": "🧐 Critic is reviewing the report…",
        "critic": "✅ Finalising…",
    }

    def on_step(name, pipeline_state):
        st.session_state.results = {
            ui: pipeline_state[key]
            for key, ui in STATE_TO_UI.items()
            if key in pipeline_state
        }
        status_box.info(STEP_MESSAGES.get(name, "Working…"))

    try:
        final_state = run_research_pipeline(topic_val, on_step=on_step)
        st.session_state.results = {
            ui: final_state[key] for key, ui in STATE_TO_UI.items() if key in final_state
        }
